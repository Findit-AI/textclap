...
│   └── clap_model.rs                 # ~280 lines: Clap + Embedding + NORM_BUDGET + LabeledScore[Owned]
│                                     # (named clap_model to avoid shadowing the `clap` CLI crate)
├── tests/
...
The first non-blank line under "Pinned revision" is `Revision: <SHA>`, in a format consumable by a
one-line `awk` parser (used by the CI workflow at Task 31). Don't reformat that line.
...
Revision: <HF revision SHA from Task 1 step 1>
...
```text
<sha-of-audio-model>  audio_model_quantized.onnx
<sha-of-text-model>   text_model_quantized.onnx
<sha-of-tokenizer>    tokenizer.json
```
...
- [ ] **Step 3a: Write `tests/fixtures/MODELS.sha256`** (sidecar in native `sha256sum -c` format):
...
Replace each `<sha-of-...>` placeholder with the literal hex digest from Task 1 step 1's `shasum -a 256`
output. Leave **two spaces** between hash and filename (BSD `sha256sum -c` requires this).
...
```bash
git add tests/fixtures/MODELS.md tests/fixtures/MODELS.sha256 \
        tests/fixtures/README.md tests/fixtures/sample.wav
...
cp "$SILERO/build.rs" ./build.rs
```

The `${SILERO:-../silero}` default works for the Findit-AI workspace layout (sibling crates as peer
directories under the same parent); override `SILERO` if your checkout differs.
...
### Task 10: `src/clap_model.rs` Embedding type + NORM_BUDGET (TDD)

**Files:**
- Create: `src/clap_model.rs` (initial — only `Embedding` and `NORM_BUDGET`; the `Clap` struct lands in Task 12 / Task 25)
...
**Files:**
- Modify: `src/clap_model.rs` (append the two types + tests)

- [ ] **Step 1: Append `LabeledScore` types to `src/clap_model.rs`**
...
- [ ] **Step 2: Append unit tests to the existing `tests` module in `src/clap_model.rs`**
...
- Create: `src/audio.rs`
- Create: `src/text.rs`
- Modify: `src/clap_model.rs` (append `Clap` struct and impl)
...
- [ ] **Step 4: Add `Clap` struct + impl to `src/clap_model.rs`**

Add the new `use` clauses **at the top of the file alongside existing imports** (not appended
mid-file):
...
Then append the `Clap` struct and its impl at the bottom of the file:
...
```bash
git add src/audio.rs src/text.rs src/mel.rs src/clap_model.rs
...
mod tests {
    ...
    ///     is NOT exactly 1 — but `sin²(π · 512 / 1024)` gives exactly 1 if we use the alternative
    ///     `0.5 · (1 − cos(2π·k/n))` formula at the symmetry axis. The periodic form has its peak
    ///     at the closer integer index. We assert the peak is between win[511] and win[513],
    ///     and win[1024 − k] = win[k − 1] (asymmetric one-step shift from the symmetric form).
    ...
    fn hann_window_periodic_length_1024() {
        ...
        // Endpoint invariants.
        assert_eq!(win[0], 0.0);
        // win[1023] is small-but-positive in periodic; symmetric would be ~0.
        assert!(win[1023] > 5e-3 && win[1023] < 2e-2,
            "periodic Hann last sample should be ~9.6e-3; got {}", win[1023]);
...
impl AudioEncoder {
    /// Load from a file path.
    ///
    /// Session-construction chain matches silero `src/session.rs:97-104` byte-for-byte: the
    /// constructor is `Session::builder()`, NOT `SessionBuilder::new()` (which doesn't exist in
    /// `ort 2.0.0-rc.12`).
...
impl TextEncoder {
    pub fn from_files<P: AsRef<Path>>(
        ...
        // Session::builder() — see Task 17 / silero session.rs:97 for the canonical chain.
        let session = Session::builder()
...
Append to `src/clap_model.rs` `tests` module:
  ...
  integration:
    ...
    steps:
      ...
      - name: Download models
        ...
        run: |
          mkdir -p ~/textclap-models
          # HF revision pin: parsed from the `Revision: <sha>` line in MODELS.md.
          REV=$(awk '/^Revision:/ {print $2; exit}' tests/fixtures/MODELS.md)
          if [ -z "$REV" ]; then echo "Could not parse Revision: from MODELS.md" >&2; exit 1; fi
          ...
          # Verify SHA256s using the native sha256sum sidecar (avoids markdown-parsing fragility).
          cd ~/textclap-models
          sha256sum -c "$GITHUB_WORKSPACE/tests/fixtures/MODELS.sha256"
